import { generateMeal } from "@utils/gameLogic/generateMeal.js";
...
describe("generateMeal", () => {
  test("returns 6 items: 1 drink, 1 main, 4 from fruit/veggie/snack", async () => {
    const meal = await generateMeal(fakeCredentials);
  ...
  test("returns different meals on multiple calls (randomized)", async () => {
    const meals = await Promise.all(Array.from({ length: 10 }, () => generateMeal(fakeCredentials)));
...
describe("calculateNutritionScore", () => {
  test("returns score 0-100 with 4 breakdown categories 0-25 each", async () => {
    const meal = await generateMeal(fakeCredentials);
    const result = await calculateNutritionScore(
      fakeCredentials,
      meal.map((i) => i.itemId),
    );
