...
**Secrets:** no plaintext committed. `lunch-swap0-sealedsecret.yaml` holds only ciphertext
(sealed strict-scope for namespace sdk-apps-dev); the controller unseals it into
`Secret lunch-swap0-secrets`, consumed via `envFrom`. Non-secret env (incl. public
`INTERACTIVE_KEY`) lives in `lunch-swap0-config`.

## Environment

| Env | Service | Namespace | Host | Health |
| --- | ------- | --------- | ---- | ------ |
| `dev` | `lunch-swap0` | `sdk-apps-dev` | lunch-swap0-dev-topia.topia-rtsdk.com | `/api/system/health` |
